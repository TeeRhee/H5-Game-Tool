...
const checks = [
  ...
  { label: "Image ratio and fit do not inherit across second-level tabs", actual: template.requiredRendererBehavior.some((rule) => rule.includes("Do not reuse an imageRatio or imageFit selected for one second-level tab")), expected: true },
  ...
  { label: "DescribeCard imageFit contain", actual: describeCardComponent?.props?.imageFit?.values?.includes("contain"), expected: true },
  ...
  { label: "DescribeCard sizing priority rule", actual: describeCardComponent?.observedStructure?.rule?.includes("page/template layout requirements") && describeCardComponent?.observedStructure?.rule?.includes("take priority"), expected: true },
  ...
  { label: "Guide with nav and info layout", actual: guideDocumentLayouts.withNavAndInfo, expected: { pageNodeId: "304:18296", breadcrumbs: { bodyRelativeX: 32, bodyRelativeY: 16, width: 188, height: 26 }, articleLayout: { bodyRelativeX: 32, bodyRelativeY: 62, width: 936, height: 607.9389, gap: 20 }, leftNav: { width: 96, height: 607.9389 }, articleContent: { layoutRelativeX: 116, width: 540, height: 607.9389, gap: 12 }, rightInfo: { layoutRelativeX: 676, width: 260, observedExampleHeight: 204, heightRule: "Observed Figma sample height is 204, but rendered sidebar height must grow downward with the real related-info item count instead of capping at four rows.", titleHeight: 36, titleDivider: { x: 0, y: 36, width: 260, height: 1, strokeColor: "rgba(255,255,255,0.10)", strokeToken: "var(--color-stroke-area)" }, listY: 44, observedExampleListHeight: 160, listHeightRule: "Observed Figma sample list area is 160 high for four visible rows, but rendered output must keep stacking additional related-info items downward instead of clipping to that sample height.", listItemHeight: 34, listGap: 8 } } },
  { label: "Guide with info layout", actual: guideDocumentLayouts.withInfo, expected: { pageNodeId: "306:4332", breadcrumbs: { bodyRelativeX: 32, bodyRelativeY: 16, width: 188, height: 26 }, articleLayout: { bodyRelativeX: 32, bodyRelativeY: 62, width: 936, height: 607.9389, gap: 20 }, articleContent: { layoutRelativeX: 0, width: 656, height: 607.9389, gap: 12 }, rightInfo: { layoutRelativeX: 676, width: 260, observedExampleHeight: 204, heightRule: "Observed Figma sample height is 204, but rendered sidebar height must grow downward with the real related-info item count instead of capping at four rows.", titleHeight: 36, titleDivider: { x: 0, y: 36, width: 260, height: 1, strokeColor: "rgba(255,255,255,0.10)", strokeToken: "var(--color-stroke-area)" }, listY: 44, observedExampleListHeight: 160, listHeightRule: "Observed Figma sample list area is 160 high for four visible rows, but rendered output must keep stacking additional related-info items downward instead of clipping to that sample height.", listItemHeight: 34, listGap: 8 } } },
  ...
  { label: "Guide related info mapping is not capped to four items", actual: mapping.rules.some((rule) => rule.includes("RelatedInfoSidebar") && rule.includes("not source data") && rule.includes("clipping or limiting the list to four")), expected: true },
